...
app.get("/random", (req, res) => {
  var randomIndexInJokes = Math.floor(Math.random() * jokes.length);
  res.json(jokes[randomIndexInJokes]);
});
//2. GET a specific joke
app.get("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  var targetJoke = jokes.find((joke) => joke.id === id);
  res.json(targetJoke);
});
...
app.get("/filter", (req, res) => {
  ...
  if (filteredJokes.length === 0) {
    res.json("this joke type does not exist.");
  }
...
app.put("/jokes/:id", (req, res) => {
  ...
  var targetJokeIndex = jokes.findIndex((joke) => joke.id === id);
  jokes[targetJokeIndex] = editedJoke;
  res.json(editedJoke);
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (targetJokeIndex > -1) {
    ...
    res.statusCode(404);
    res.json(`non-existent joke id ${id}`);
  }
});
//8. DELETE All jokes
